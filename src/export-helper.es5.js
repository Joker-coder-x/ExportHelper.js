    ...
    var CSV = (function() {
      ...
      CSV.prototype.serialize = {
        object: function object(_object) {
          var that = this,
            attributes = Object.keys(_object),
            serialized = Array(attributes.length);
          forEach(attributes, function(attr, index) {
            serialized[index] = that[serializeType(_object[attr])](
              _object[attr]
            );
          });
          return serialized;
        ...
        array: function array(_array) {
          var that = this,
            serialized = Array(_array.length);
          forEach(_array, function(value, index) {
            serialized[index] = that[serializeType(value)](value);
          });
          return serialized;
        },
        string: function string(_string2) {
          return '"' + String(_string2).replace(/"/g, '""') + '"';
        },
      ...
      CSV.prototype.encode = function(callback) {
        ...
        if (!callback) {
          ...
          callback = function callback(record, index) {
